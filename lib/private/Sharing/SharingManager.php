...
final readonly class SharingManager implements ISharingManager, IEventListener {
	...
	public function updateShareRecipientPermission(ShareAccessContext $accessContext, Share $share, ShareRecipient $recipient, SharePermission $permission): Share {
		...
		$enabledPermissions = $share->getEnabledPermissions($accessContext);
	...
	private function validatePermission(ShareAccessContext $accessContext, Share $share, string $permissionTypeClass): void {
		if (!isset($share->getEnabledPermissions($accessContext)[$permissionTypeClass])) {
			throw new ShareOperationForbiddenException();
		}
	}
	...
	private function validateInteraction(ShareAccessContext $accessContext, Share $share): void {
		$action = new ShareAction(
			null, array_values(array_map(static fn (SharePermission $permission): string => $permission->class, $share->getEnabledPermissions($accessContext)))
		);
	...
	private function assertShareCanBeActive(Share $share): void {
		...
		if ($share->getEnabledPermissions(new ShareAccessContext(overrideChecks: true)) === []) {
