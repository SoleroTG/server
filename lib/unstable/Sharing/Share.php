...
final class Share {
	 ...
	 * @experimental 35.0.0
	 */
	public function getEnabledPermissions(ShareAccessContext $accessContext): array {
		...
		if (!$accessContext->overrideChecks && !$this->owner->isCurrentUser($accessContext)) {
			...
			foreach ($this->getRecipients($accessContext) as $recipient) {
			...
			foreach ($recipientsEnabledPermissions as $permission) {
				// We only allow removing permissions. Recipients can't be granted more permissions than the share itself grants.
	 ...
	 * @experimental 35.0.0
	 */
	public function getRecipients(ShareAccessContext $accessContext): array {
		$hash = $accessContext->getHash();
	...
	public function format(ISharingRegistry $registry, IFactory $l10nFactory, IURLGenerator $urlGenerator, IUserManager $userManager, ShareAccessContext $accessContext): array {
		...
		$enabledPermissionTypeClasses = array_values(array_map(static fn (SharePermission $permission): string => $permission->class, $this->getEnabledPermissions($accessContext)));
	...
	public static function formatMultiple(ISharingRegistry $registry, IFactory $l10nFactory, IURLGenerator $urlGenerator, IUserManager $userManager, ShareAccessContext $accessContext, array $shares): array {
		...
		usort($shares, static fn (Share $a, Share $b): int => 2 * (count($b->getEnabledPermissions($accessContext)) <=> count($a->getEnabledPermissions($accessContext))) + ($a->id <=> $b->id));
