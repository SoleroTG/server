...
final class SharingManagerTest extends AbstractSharingManagerTests {
	private function assertShareSyncedWithDb(ShareAccessContext $accessContext, Share $share): void {
		...
		foreach ([$retrieved, $share] as $item) {
			$item->getEnabledPermissions(new ShareAccessContext(overrideChecks: true));
			$item->getEnabledPermissions(new ShareAccessContext($this->owner));
			$item->getEnabledPermissions(new ShareAccessContext($this->user1));
			$item->getEnabledPermissions(new ShareAccessContext($this->user2));
			foreach ($item->recipients as $recipient) {
				$recipient->getEnabledPermissions();
			}
		}
